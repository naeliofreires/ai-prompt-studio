...
import type { GeneratePromptPayload } from "../../../src/features/prompt-generation/contract/ipc";
import { promptStudioClient } from "../../../src/features/prompt-generation/ui/api/prompt-studio-client";
...
describe("promptStudioClient", () => {
  ...
  it("1. forwards prompt attachments through the prompt studio client ", () => {
    ...
    return promptStudioClient.generatePrompt(payload).then(() => {
      expect(bridge.promptGeneration.generatePrompt).toHaveBeenCalledWith(payload);
    });
  });
});
