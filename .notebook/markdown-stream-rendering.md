...
Entry: `src/ui/components/StreamingMarkdown/index.tsx:StreamingMarkdown()`

Markdown:
- `StreamingMarkdown()` stabilizes partial content via `src/ui/markdown/stabilizeStreamingMarkdown.ts`
- `src/ui/markdown/createMarkdownPlugins.ts` applies `remark-gfm` and `rehype-sanitize`
...
Streaming hook:
- `src/ui/hooks/useStreamedMarkdown.ts:consume()` handles `ReadableStream` before async iterable detection
