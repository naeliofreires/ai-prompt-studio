import type { LlmAdapter } from "../../shared/contracts/llm.js";
import type { PromptEvaluation } from "../../shared/domain/prompt-evaluation.js";
import type { ProviderId } from "../../shared/domain/provider.js";
import type { SavePromptSessionInput } from "../../shared/domain/prompt-session.js";
...
export async function generateRefinedPrompt(
  ...
  try {
    ...
    const evaluation = await evaluatePromptIfAvailable(input, personaContext, out.prompt, dependencies);
    if (evaluation) {
      result.evaluation = evaluation;
    }

    await savePromptSessionIfAvailable(input, out.prompt, result.evaluation, out.tokensUsed, dependencies);
...
async function savePromptSessionIfAvailable(
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : "Prompt session could not be saved.";
...
async function evaluatePromptIfAvailable(
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : "Prompt evaluation failed.";
