...
export function resolveLanguageModel(providerId: string, model: string): LanguageModelV3 {
  ...
  const baseUrl = provider.defaultBaseUrl
    ? (process.env[`${providerId.toUpperCase()}_BASE_URL`]?.trim() || provider.defaultBaseUrl)
    : undefined;
  ...
  switch (provider.sdkType) {
    ...
    default:
      throw new Error(`Unhandled SDK type for provider: ${provider.provider} (sdkType: ${provider.sdkType ?? "undefined"})`);
  }
