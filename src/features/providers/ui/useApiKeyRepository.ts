...
export function useApiKeyRepository() {
  ...
  const isConfigured = useCallback(
    (providerId: ProviderId) => apiKeyRepository.isConfigured(providerId),
    [],
  );

  const saveKeys = useCallback((patch: Partial<Record<ProviderId, string>>) => {
  ...
  const clearProvider = useCallback((id: ProviderId) => {
